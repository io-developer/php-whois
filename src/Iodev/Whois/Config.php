<?php

namespace Iodev\Whois;

class Config
{
    /**
     * @return array
     */
    public static function getServersData()
    {
        return [
            [ "zone" => ".abogado", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".ac", "host" => "whois.nic.ac" ],
            [ "zone" => ".ac.ac", "host" => "whois.nic.ac" ],
            [ "zone" => ".ac.at", "host" => "whois.nic.at" ],
            [ "zone" => ".ac.be", "host" => "whois.dns.be" ],
            [ "zone" => ".ac.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".ac.il", "host" => "whois.isoc.org.il" ],
            [ "zone" => ".ac.in", "host" => "whois.inregistry.in" ],
            [ "zone" => ".ac.jp", "host" => "whois.nic.ad.jp" ],
            [ "zone" => ".ac.ke", "host" => "whois.kenic.or.ke" ],
            [ "zone" => ".ac.nz", "host" => "whois.srs.net.nz" ],
            [ "zone" => ".ac.th", "host" => "whois.thnic.net" ],
            [ "zone" => ".ac.uk", "host" => "whois.ja.net" ],
            [ "zone" => ".ac.za", "host" => "whois.co.za" ],
            [ "zone" => ".academy", "host" => "whois.donuts.co" ],
            [ "zone" => ".accountants", "host" => "whois.donuts.co" ],
            [ "zone" => ".active", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".actor", "host" => "whois.unitedtld.com" ],
            [ "zone" => ".ad", "host" => "whois.ripe.net" ],
            [ "zone" => ".adm.br", "host" => "whois.nic.br" ],
            [ "zone" => ".adult", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".adv.br", "host" => "whois.nic.br" ],
            [ "zone" => ".ae", "host" => "whois-check.aeda.net.ae", "parser_type" => "common_flat" ],
            [ "zone" => ".ae", "host" => "whois.aeda.net.ae", "parser_type" => "common_flat" ],
            [ "zone" => ".ae.com", "host" => "whois.centralnic.net" ],
            [ "zone" => ".aero", "host" => "whois.aero" ],
            [ "zone" => ".af", "host" => "whois.netnames.net" ],
            [ "zone" => ".af", "host" => "whois.nic.af" ],
            [ "zone" => ".ag", "host" => "whois.nic.ag" ],
            [ "zone" => ".agency", "host" => "whois.donuts.co" ],
            [ "zone" => ".agro.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".ah.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".ai", "host" => "whois.ai" ],
            [ "zone" => ".aid.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".airforce", "host" => "whois.unitedtld.com" ],
            [ "zone" => ".al", "host" => "whois.ripe.net" ],
            [ "zone" => ".allfinanz", "host" => "whois.ksregistry.net" ],
            [ "zone" => ".alsace", "host" => "whois-alsace.nic.fr" ],
            [ "zone" => ".alt.za", "host" => "whois.co.za" ],
            [ "zone" => ".am", "host" => "whois.amnic.net" ],
            [ "zone" => ".am", "host" => "whois.nic.am" ],
            [ "zone" => ".am.br", "host" => "whois.nic.br" ],
            [ "zone" => ".android", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".ao", "host" => "whois.reg.it.ao" ],
            [ "zone" => ".apartments", "host" => "whois.donuts.co" ],
            [ "zone" => ".aquarelle", "host" => "whois-aquarelle.nic.fr" ],
            [ "zone" => ".archi", "host" => "whois.ksregistry.net" ],
            [ "zone" => ".army", "host" => "whois.rightside.co" ],
            [ "zone" => ".arpa", "host" => "whois.iana.org" ],
            [ "zone" => ".arq.br", "host" => "whois.nic.br" ],
            [ "zone" => ".art.br", "host" => "whois.nic.br" ],
            [ "zone" => ".as", "host" => "whois.nic.as" ],
            [ "zone" => ".asia", "host" => "whois.nic.asia" ],
            [ "zone" => ".asso.fr", "host" => "whois.nic.fr" ],
            [ "zone" => ".asso.mc", "host" => "whois.ripe.net" ],
            [ "zone" => ".associates", "host" => "whois.donuts.co" ],
            [ "zone" => ".at", "host" => "whois.nic.at" ],
            [ "zone" => ".atm.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".attorney", "host" => "whois.rightside.co" ],
            [ "zone" => ".au", "host" => "whois.audns.net.au", "parser_type" => "common_flat" ],
            [ "zone" => ".au", "host" => "whois.aunic.net", "parser_type" => "common_flat" ],
            [ "zone" => ".auction", "host" => "whois.donuts.co" ],
            [ "zone" => ".auction", "host" => "whois.unitedtld.com" ],
            [ "zone" => ".audio", "host" => "whois.uniregistry.net" ],
            [ "zone" => ".auto.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".autos", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".aw", "host" => "whois.nic.aw" ],
            [ "zone" => ".ax", "host" => "whois.ax" ],
            [ "zone" => ".band", "host" => "whois.rightside.co" ],
            [ "zone" => ".bank", "host" => "whois.nic.bank" ],
            [ "zone" => ".bar", "host" => "whois.nic.bar" ],
            [ "zone" => ".barclaycard", "host" => "whois.nic.barclaycard" ],
            [ "zone" => ".barclays", "host" => "whois.nic.barclays" ],
            [ "zone" => ".bargains", "host" => "whois.donuts.co" ],
            [ "zone" => ".bayern", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".bbs.tr", "host" => "whois.metu.edu.tr" ],
            [ "zone" => ".be", "host" => "whois.dns.be" ],
            [ "zone" => ".beer", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".berlin", "host" => "whois.nic.berlin" ],
            [ "zone" => ".best", "host" => "whois.nic.best" ],
            [ "zone" => ".bg", "host" => "whois.register.bg" ],
            [ "zone" => ".bh", "host" => "whois.nic.bh" ],
            [ "zone" => ".bi", "host" => "whois.nic.bi" ],
            [ "zone" => ".bi", "host" => "whois1.nic.bi" ],
            [ "zone" => ".bid", "host" => "whois.nic.bid" ],
            [ "zone" => ".bike", "host" => "whois.donuts.co" ],
            [ "zone" => ".bingo", "host" => "whois.donuts.co" ],
            [ "zone" => ".bio", "host" => "whois.ksregistry.net" ],
            [ "zone" => ".bio.br", "host" => "whois.nic.br" ],
            [ "zone" => ".biz", "host" => "whois.biz" ],
            [ "zone" => ".biz", "host" => "whois.neulevel.biz" ],
            [ "zone" => ".biz", "host" => "whois.nic.biz" ],
            [ "zone" => ".biz.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".bj", "host" => "whois.nic.bj" ],
            [ "zone" => ".bj", "host" => "www.nic.bj" ],
            [ "zone" => ".bj.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".black", "host" => "whois.afilias.net" ],
            [ "zone" => ".blackfriday", "host" => "whois.uniregistry.net" ],
            [ "zone" => ".blue", "host" => "whois.afilias.net" ],
            [ "zone" => ".bmw", "host" => "whois.ksregistry.net" ],
            [ "zone" => ".bn", "host" => "whois.bn" ],
            [ "zone" => ".bnpparibas", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".bo", "host" => "whois.nic.bo" ],
            [ "zone" => ".boo", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".boutique", "host" => "whois.donuts.co" ],
            [ "zone" => ".br", "host" => "whois.nic.br" ],
            [ "zone" => ".br", "host" => "whois.registro.br" ],
            [ "zone" => ".br.com", "host" => "whois.centralnic.com" ],
            [ "zone" => ".br.com", "host" => "whois.centralnic.net" ],
            [ "zone" => ".brussels", "host" => "whois.nic.brussels" ],
            [ "zone" => ".bt", "host" => "whois.netnames.net" ],
            [ "zone" => ".budapest", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".build", "host" => "whois.nic.build" ],
            [ "zone" => ".build", "host" => "whois.nic.build " ],
            [ "zone" => ".builders", "host" => "whois.donuts.co" ],
            [ "zone" => ".business", "host" => "whois.donuts.co" ],
            [ "zone" => ".buzz", "host" => "whois.nic.buzz" ],
            [ "zone" => ".bw", "host" => "whois.nic.net.bw" ],
            [ "zone" => ".by", "host" => "whois.cctld.by" ],
            [ "zone" => ".bz", "host" => "whois.afilias-grs.info." ],
            [ "zone" => ".bz", "host" => "whois.belizenic.bz" ],
            [ "zone" => ".bzh", "host" => "whois-bzh.nic.fr" ],
            [ "zone" => ".ca", "host" => "whois.cira.ca" ],
            [ "zone" => ".cab", "host" => "whois.donuts.co" ],
            [ "zone" => ".cal", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".camera", "host" => "whois.donuts.co" ],
            [ "zone" => ".camp", "host" => "whois.donuts.co" ],
            [ "zone" => ".cancerresearch", "host" => "whois.nic.cancerresearch" ],
            [ "zone" => ".canon", "host" => "whois.nic.canon" ],
            [ "zone" => ".capetown", "host" => "capetown-whois.registry.net.za" ],
            [ "zone" => ".capital", "host" => "whois.donuts.co" ],
            [ "zone" => ".cards", "host" => "whois.donuts.co" ],
            [ "zone" => ".care", "host" => "whois.donuts.co" ],
            [ "zone" => ".career", "host" => "whois.nic.career" ],
            [ "zone" => ".careers", "host" => "whois.donuts.co" ],
            [ "zone" => ".casa", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".cash", "host" => "whois.donuts.co" ],
            [ "zone" => ".casino", "host" => "whois.donuts.co" ],
            [ "zone" => ".cat", "host" => "whois.cat" ],
            [ "zone" => ".catering", "host" => "whois.donuts.co" ],
            [ "zone" => ".cc", "host" => "ccwhois.verisign-grs.com" ],
            [ "zone" => ".cc", "host" => "whois.nic.cc" ],
            [ "zone" => ".center", "host" => "whois.donuts.co" ],
            [ "zone" => ".ceo", "host" => "whois.nic.ceo" ],
            [ "zone" => ".cern", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".cf", "host" => "whois.dot.cf" ],
            [ "zone" => ".ch", "host" => "whois.nic.ch" ],
            [ "zone" => ".channel", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".chat", "host" => "whois.donuts.co" ],
            [ "zone" => ".cheap", "host" => "whois.donuts.co" ],
            [ "zone" => ".christmas", "host" => "whois.uniregistry.net" ],
            [ "zone" => ".chrome", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".church", "host" => "whois.donuts.co" ],
            [ "zone" => ".ci", "host" => "whois.nic.ci" ],
            [ "zone" => ".city", "host" => "whois.donuts.co" ],
            [ "zone" => ".cl", "host" => "whois.nic.cl" ],
            [ "zone" => ".claims", "host" => "whois.donuts.co" ],
            [ "zone" => ".cleaning", "host" => "whois.donuts.co" ],
            [ "zone" => ".click", "host" => "whois.uniregistry.net" ],
            [ "zone" => ".clinic", "host" => "whois.donuts.co" ],
            [ "zone" => ".clothing", "host" => "whois.donuts.co" ],
            [ "zone" => ".cloud", "host" => "whois.nic.cloud" ],
            [ "zone" => ".club", "host" => "whois.nic.club" ],
            [ "zone" => ".cm", "host" => "whois.netcom.cm" ],
            [ "zone" => ".cn", "host" => "whois.cnnic.cn" ],
            [ "zone" => ".cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".cn.com", "host" => "whois.centralnic.com" ],
            [ "zone" => ".cn.com", "host" => "whois.centralnic.net" ],
            [ "zone" => ".cng.br", "host" => "whois.nic.br" ],
            [ "zone" => ".cnt.br", "host" => "whois.nic.br" ],
            [ "zone" => ".co", "host" => "whois.nic.co" ],
            [ "zone" => ".co.ac", "host" => "whois.nic.ac" ],
            [ "zone" => ".co.at", "host" => "whois.nic.at" ],
            [ "zone" => ".co.il", "host" => "whois.isoc.org.il" ],
            [ "zone" => ".co.in", "host" => "whois.inregistry.in" ],
            [ "zone" => ".co.jp", "host" => "whois.nic.ad.jp" ],
            [ "zone" => ".co.ke", "host" => "whois.kenic.or.ke" ],
            [ "zone" => ".co.nl", "host" => "whois.co.nl" ],
            [ "zone" => ".co.nz", "host" => "whois.srs.net.nz" ],
            [ "zone" => ".co.th", "host" => "whois.thnic.net" ],
            [ "zone" => ".co.uk", "host" => "whois.nic.uk" ],
            [ "zone" => ".co.ve", "host" => "whois.nic.ve" ],
            [ "zone" => ".coach", "host" => "whois.donuts.co" ],
            [ "zone" => ".codes", "host" => "whois.donuts.co" ],
            [ "zone" => ".coffee", "host" => "whois.donuts.co" ],
            [ "zone" => ".college", "host" => "whois.centralnic.com" ],
            [ "zone" => ".college", "host" => "whois.nic.college" ],
            [ "zone" => ".cologne", "host" => "whois-fe1.pdt.cologne.tango.knipp.de" ],
            [ "zone" => ".com", "host" => "whois.crsnic.net" ],
            [ "zone" => ".com", "host" => "whois.verisign-grs.com" ],
            [ "zone" => ".com.br", "host" => "whois.nic.br" ],
            [ "zone" => ".com.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".com.co", "host" => "whois.nic.co" ],
            [ "zone" => ".com.de", "host" => "whois.centralnic.com" ],
            [ "zone" => ".com.ec", "host" => "whois.lac.net" ],
            [ "zone" => ".com.fr", "host" => "whois.nic.fr" ],
            [ "zone" => ".com.hk", "host" => "whois.hkdnr.net.hk" ],
            [ "zone" => ".com.mm", "host" => "whois.nic.mm" ],
            [ "zone" => ".com.mx", "host" => "whois.nic.mx" ],
            [ "zone" => ".com.my", "host" => "whois.mynic.net.my" ],
            [ "zone" => ".com.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".com.pt", "host" => "whois.dns.pt" ],
            [ "zone" => ".com.ru", "host" => "whois.nic.ru" ],
            [ "zone" => ".com.ru", "host" => "whois.ripn.net" ],
            [ "zone" => ".com.sg", "host" => "whois.nic.net.sg" ],
            [ "zone" => ".com.tr", "host" => "whois.metu.edu.tr" ],
            [ "zone" => ".com.tw", "host" => "whois.twnic.net" ],
            [ "zone" => ".com.ua", "host" => "whois.net.ua" ],
            [ "zone" => ".com.ve", "host" => "whois.nic.ve" ],
            [ "zone" => ".community", "host" => "whois.donuts.co" ],
            [ "zone" => ".company", "host" => "whois.donuts.co" ],
            [ "zone" => ".computer", "host" => "whois.donuts.co" ],
            [ "zone" => ".condos", "host" => "whois.donuts.co" ],
            [ "zone" => ".construction", "host" => "whois.donuts.co" ],
            [ "zone" => ".consulting", "host" => "whois.unitedtld.com" ],
            [ "zone" => ".contractors", "host" => "whois.donuts.co" ],
            [ "zone" => ".cooking", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".cool", "host" => "whois.donuts.co" ],
            [ "zone" => ".coop", "host" => "whois.nic.coop" ],
            [ "zone" => ".country", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".courses", "host" => "whois.aridnrs.net.au" ],
            [ "zone" => ".cq.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".cr", "host" => "whois.nic.cr" ],
            [ "zone" => ".credit", "host" => "whois.donuts.co" ],
            [ "zone" => ".creditcard", "host" => "whois.donuts.co" ],
            [ "zone" => ".cricket", "host" => "whois.nic.cricket" ],
            [ "zone" => ".cruises", "host" => "whois.donuts.co" ],
            [ "zone" => ".cuisinella", "host" => "whois.nic.cuisinella" ],
            [ "zone" => ".cx", "host" => "whois.nic.cx" ],
            [ "zone" => ".cymru", "host" => "whois.nic.cymru" ],
            [ "zone" => ".cz", "host" => "whois.nic.cz", "parser_type" => "block" ],
            [ "zone" => ".dad", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".dance", "host" => "whois.unitedtld.com" ],
            [ "zone" => ".dating", "host" => "whois.donuts.co" ],
            [ "zone" => ".datsun", "host" => "whois.nic.gmo" ],
            [ "zone" => ".day", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".dclk", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".de", "host" => "whois.denic.de", "parser_type" => "block" ],
            [ "zone" => ".de.com", "host" => "whois.centralnic.net" ],
            [ "zone" => ".deals", "host" => "whois.donuts.co" ],
            [ "zone" => ".degree", "host" => "whois.rightside.co" ],
            [ "zone" => ".delivery", "host" => "whois.donuts.co" ],
            [ "zone" => ".democrat", "host" => "whois.unitedtld.com" ],
            [ "zone" => ".dental", "host" => "whois.donuts.co" ],
            [ "zone" => ".dentist", "host" => "whois.rightside.co" ],
            [ "zone" => ".desi", "host" => "whois.ksregistry.net" ],
            [ "zone" => ".design", "host" => "whois.nic.design" ],
            [ "zone" => ".dev", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".diamonds", "host" => "whois.donuts.co" ],
            [ "zone" => ".diet", "host" => "whois.uniregistry.net" ],
            [ "zone" => ".digital", "host" => "whois.donuts.co" ],
            [ "zone" => ".direct", "host" => "whois.donuts.co" ],
            [ "zone" => ".directory", "host" => "whois.donuts.co" ],
            [ "zone" => ".discount", "host" => "whois.donuts.co" ],
            [ "zone" => ".dk", "host" => "whois.dk-hostmaster.dk" ],
            [ "zone" => ".dm", "host" => "whois.nic.cx", "parser_type" => "common_flat" ],
            [ "zone" => ".dm", "host" => "whois.nic.dm", "parser_type" => "common_flat" ],
            [ "zone" => ".dn.ua", "host" => "whois.net.ua" ],
            [ "zone" => ".do", "host" => "whois.nic.do" ],
            [ "zone" => ".docs", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".domains", "host" => "whois.donuts.co" ],
            [ "zone" => ".doosan", "host" => "whois.nic.doosan" ],
            [ "zone" => ".durban", "host" => "durban-whois.registry.net.za" ],
            [ "zone" => ".dvag", "host" => "whois.ksregistry.net" ],
            [ "zone" => ".dz", "host" => "whois.nic.dz" ],
            [ "zone" => ".eat", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".ec", "host" => "whois.nic.ec" ],
            [ "zone" => ".ecn.br", "host" => "whois.nic.br" ],
            [ "zone" => ".edu", "host" => "whois.educause.edu" ],
            [ "zone" => ".edu", "host" => "whois.educause.net" ],
            [ "zone" => ".edu", "host" => "whois.internic.net" ],
            [ "zone" => ".edu.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".edu.hk", "host" => "whois.hkdnr.net.hk" ],
            [ "zone" => ".edu.mm", "host" => "whois.nic.mm" ],
            [ "zone" => ".edu.mx", "host" => "whois.nic.mx" ],
            [ "zone" => ".edu.my", "host" => "whois.mynic.net.my" ],
            [ "zone" => ".edu.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".edu.pt", "host" => "whois.dns.pt" ],
            [ "zone" => ".edu.sg", "host" => "whois.nic.net.sg" ],
            [ "zone" => ".edu.tr", "host" => "whois.metu.edu.tr" ],
            [ "zone" => ".edu.za", "host" => "whois.co.za" ],
            [ "zone" => ".education", "host" => "whois.donuts.co" ],
            [ "zone" => ".ee", "host" => "whois.tld.ee" ],
            [ "zone" => ".email", "host" => "whois.donuts.co" ],
            [ "zone" => ".emerck", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".energy", "host" => "whois.donuts.co" ],
            [ "zone" => ".eng.br", "host" => "whois.nic.br" ],
            [ "zone" => ".engineer", "host" => "whois.rightside.co" ],
            [ "zone" => ".engineering", "host" => "whois.donuts.co" ],
            [ "zone" => ".enterprises", "host" => "whois.donuts.co" ],
            [ "zone" => ".epson", "host" => "whois.aridnrs.net.au" ],
            [ "zone" => ".equipment", "host" => "whois.donuts.co" ],
            [ "zone" => ".ernet.in", "host" => "whois.inregistry.in" ],
            [ "zone" => ".esp.br", "host" => "whois.nic.br" ],
            [ "zone" => ".esq", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".estate", "host" => "whois.donuts.co" ],
            [ "zone" => ".etc.br", "host" => "whois.nic.br" ],
            [ "zone" => ".eti.br", "host" => "whois.nic.br" ],
            [ "zone" => ".eu", "host" => "whois.eu" ],
            [ "zone" => ".eu.com", "host" => "whois.centralnic.com" ],
            [ "zone" => ".eu.com", "host" => "whois.centralnic.net" ],
            [ "zone" => ".eu.lv", "host" => "whois.biz" ],
            [ "zone" => ".eurovision", "host" => "whois.nic.eurovision" ],
            [ "zone" => ".eus", "host" => "whois.eus.coreregistry.net" ],
            [ "zone" => ".eus", "host" => "whois.nic.eus" ],
            [ "zone" => ".events", "host" => "whois.donuts.co" ],
            [ "zone" => ".exchange", "host" => "whois.donuts.co" ],
            [ "zone" => ".expert", "host" => "whois.donuts.co" ],
            [ "zone" => ".exposed", "host" => "whois.donuts.co" ],
            [ "zone" => ".fail", "host" => "whois.donuts.co" ],
            [ "zone" => ".fans", "host" => "whois.nic.fans" ],
            [ "zone" => ".farm", "host" => "whois.donuts.co" ],
            [ "zone" => ".fashion", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".feedback", "host" => "whois.centralnic.com" ],
            [ "zone" => ".fi", "host" => "whois.fi" ],
            [ "zone" => ".fi", "host" => "whois.ficora.fi" ],
            [ "zone" => ".fin.ec", "host" => "whois.lac.net" ],
            [ "zone" => ".finance", "host" => "whois.donuts.co" ],
            [ "zone" => ".financial", "host" => "whois.donuts.co" ],
            [ "zone" => ".firmdale", "host" => "whois.nic.firmdale" ],
            [ "zone" => ".fish", "host" => "whois.donuts.co" ],
            [ "zone" => ".fishing", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".fit", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".fitness", "host" => "whois.donuts.co" ],
            [ "zone" => ".fj", "host" => "whois.usp.ac.fj" ],
            [ "zone" => ".flights", "host" => "whois.donuts.co" ],
            [ "zone" => ".florist", "host" => "whois.donuts.co" ],
            [ "zone" => ".flowers", "host" => "whois.uniregistry.net" ],
            [ "zone" => ".flsmidth", "host" => "whois.ksregistry.net" ],
            [ "zone" => ".fly", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".fm", "host" => "whois.nic.fm" ],
            [ "zone" => ".fm.br", "host" => "whois.nic.br" ],
            [ "zone" => ".fo", "host" => "whois.nic.fo" ],
            [ "zone" => ".foo", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".football", "host" => "whois.donuts.co" ],
            [ "zone" => ".forsale", "host" => "whois.unitedtld.com" ],
            [ "zone" => ".fot.br", "host" => "whois.nic.br" ],
            [ "zone" => ".foundation", "host" => "whois.donuts.co" ],
            [ "zone" => ".fr", "host" => "whois.nic.fr" ],
            [ "zone" => ".frl", "host" => "whois.nic.frl" ],
            [ "zone" => ".frogans", "host" => "whois-frogans.nic.fr" ],
            [ "zone" => ".fst.br", "host" => "whois.nic.br" ],
            [ "zone" => ".fund", "host" => "whois.donuts.co" ],
            [ "zone" => ".furniture", "host" => "whois.donuts.co" ],
            [ "zone" => ".futbol", "host" => "whois.unitedtld.com" ],
            [ "zone" => ".g12.br", "host" => "whois.nic.br" ],
            [ "zone" => ".ga", "host" => "whois.dot.ga" ],
            [ "zone" => ".gal", "host" => "whois.gal.coreregistry.net" ],
            [ "zone" => ".gallery", "host" => "whois.donuts.co" ],
            [ "zone" => ".garden", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".gb", "host" => "whois.ripe.net" ],
            [ "zone" => ".gb.com", "host" => "whois.centralnic.com" ],
            [ "zone" => ".gb.com", "host" => "whois.centralnic.net" ],
            [ "zone" => ".gb.net", "host" => "whois.centralnic.com" ],
            [ "zone" => ".gb.net", "host" => "whois.centralnic.net" ],
            [ "zone" => ".gbiz", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".gd", "host" => "whois.nic.gd", "parser_type" => "common_flat" ],
            [ "zone" => ".gd.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".gdn", "host" => "whois.gdnregistry.com" ],
            [ "zone" => ".geek.nz", "host" => "whois.srs.net.nz" ],
            [ "zone" => ".gen.nz", "host" => "whois.srs.net.nz" ],
            [ "zone" => ".gent", "host" => "whois.nic.gent" ],
            [ "zone" => ".gf", "host" => "whois.mediaserv.net" ],
            [ "zone" => ".gg", "host" => "whois.gg" ],
            [ "zone" => ".ggee", "host" => "whois.nic.ggee" ],
            [ "zone" => ".gi", "host" => "whois2.afilias-grs.net" ],
            [ "zone" => ".gift", "host" => "whois.uniregistry.net" ],
            [ "zone" => ".gifts", "host" => "whois.donuts.co" ],
            [ "zone" => ".gives", "host" => "whois.rightside.co" ],
            [ "zone" => ".gl", "host" => "whois.nic.gl" ],
            [ "zone" => ".glass", "host" => "whois.donuts.co" ],
            [ "zone" => ".gle", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".global", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".global", "host" => "whois.nic.global" ],
            [ "zone" => ".globo", "host" => "whois.gtlds.nic.br" ],
            [ "zone" => ".gmail", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".gmina.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".gmx", "host" => "whois-fe1.gmx.tango.knipp.de" ],
            [ "zone" => ".go.id", "host" => "whois.idnic.net.id" ],
            [ "zone" => ".go.jp", "host" => "whois.nic.ad.jp" ],
            [ "zone" => ".go.th", "host" => "whois.thnic.net" ],
            [ "zone" => ".gob.mx", "host" => "whois.nic.mx" ],
            [ "zone" => ".goldpoint", "host" => "whois.nic.goldpoint" ],
            [ "zone" => ".goo", "host" => "whois.nic.gmo" ],
            [ "zone" => ".goog", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".google", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".gop", "host" => "whois-cl01.mm-registry.com" ],
            [ "zone" => ".gov", "host" => "whois.dotgov.gov" ],
            [ "zone" => ".gov", "host" => "whois.nic.gov" ],
            [ "zone" => ".gov.br", "host" => "whois.nic.br" ],
            [ "zone" => ".gov.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".gov.ec", "host" => "whois.lac.net" ],
            [ "zone" => ".gov.il", "host" => "whois.isoc.org.il" ],
            [ "zone" => ".gov.in", "host" => "whois.inregistry.in" ],
            [ "zone" => ".gov.mm", "host" => "whois.nic.mm" ],
            [ "zone" => ".gov.mx", "host" => "whois.nic.mx" ],
            [ "zone" => ".gov.my", "host" => "whois.mynic.net.my" ],
            [ "zone" => ".gov.sg", "host" => "whois.nic.net.sg" ],
            [ "zone" => ".gov.tr", "host" => "whois.metu.edu.tr" ],
            [ "zone" => ".gov.uk", "host" => "whois.ja.net" ],
            [ "zone" => ".gov.za", "host" => "whois.co.za" ],
            [ "zone" => ".gq", "host" => "whois.dominio.gq" ],
            [ "zone" => ".graphics", "host" => "whois.donuts.co" ],
            [ "zone" => ".gratis", "host" => "whois.donuts.co" ],
            [ "zone" => ".green", "host" => "whois.afilias.net" ],
            [ "zone" => ".gripe", "host" => "whois.donuts.co" ],
            [ "zone" => ".gs", "host" => "whois.nic.gs" ],
            [ "zone" => ".gs.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".gsm.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".guide", "host" => "whois.donuts.co" ],
            [ "zone" => ".guitars", "host" => "whois.uniregistry.net" ],
            [ "zone" => ".guru", "host" => "whois.donuts.co" ],
            [ "zone" => ".gv.ac", "host" => "whois.nic.ac" ],
            [ "zone" => ".gv.at", "host" => "whois.nic.at" ],
            [ "zone" => ".gx.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".gy", "host" => "whois.registry.gy" ],
            [ "zone" => ".gz.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".hamburg", "host" => "whois.nic.hamburg" ],
            [ "zone" => ".hangout", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".haus", "host" => "whois.unitedtld.com" ],
            [ "zone" => ".hb.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".he.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".healthcare", "host" => "whois.donuts.co" ],
            [ "zone" => ".help", "host" => "whois.uniregistry.net" ],
            [ "zone" => ".here", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".hi.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".hiphop", "host" => "whois.uniregistry.net" ],
            [ "zone" => ".hiv", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".hk", "host" => "whois.hkirc.hk" ],
            [ "zone" => ".hk", "host" => "whois.hknic.net.hk" ],
            [ "zone" => ".hk.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".hl.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".hm", "host" => "whois.registry.hm" ],
            [ "zone" => ".hn", "host" => "whois.nic.hn" ],
            [ "zone" => ".hn", "host" => "whois2.afilias-grs.net" ],
            [ "zone" => ".hn.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".holdings", "host" => "whois.donuts.co" ],
            [ "zone" => ".holiday", "host" => "whois.donuts.co" ],
            [ "zone" => ".homes", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".horse", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".host", "host" => "whois.centralnic.com" ],
            [ "zone" => ".host", "host" => "whois.nic.host" ],
            [ "zone" => ".hosting", "host" => "whois.uniregistry.net" ],
            [ "zone" => ".house", "host" => "whois.donuts.co" ],
            [ "zone" => ".how", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".hr", "host" => "whois.dns.hr" ],
            [ "zone" => ".ht", "host" => "whois.nic.ht" ],
            [ "zone" => ".hu", "host" => "whois.nic.hu" ],
            [ "zone" => ".hu.com", "host" => "whois.centralnic.com" ],
            [ "zone" => ".hu.com", "host" => "whois.centralnic.net" ],
            [ "zone" => ".ibm", "host" => "whois.nic.ibm" ],
            [ "zone" => ".id", "host" => "whois.pandi.or.id" ],
            [ "zone" => ".ie", "host" => "whois.domainregistry.ie" ],
            [ "zone" => ".ifm", "host" => "whois.nic.ifm" ],
            [ "zone" => ".il", "host" => "whois.isoc.org.il" ],
            [ "zone" => ".im", "host" => "whois.nic.im" ],
            [ "zone" => ".immo", "host" => "whois.donuts.co" ],
            [ "zone" => ".immobilien", "host" => "whois.unitedtld.com" ],
            [ "zone" => ".in", "host" => "whois.inregistry.in" ],
            [ "zone" => ".in", "host" => "whois.inregistry.net" ],
            [ "zone" => ".in.th", "host" => "whois.thnic.net" ],
            [ "zone" => ".ind.br", "host" => "whois.nic.br" ],
            [ "zone" => ".industries", "host" => "whois.donuts.co" ],
            [ "zone" => ".inf.br", "host" => "whois.nic.br" ],
            [ "zone" => ".infiniti", "host" => "whois.nic.gmo" ],
            [ "zone" => ".info", "host" => "whois.afilias.info" ],
            [ "zone" => ".info", "host" => "whois.afilias.net" ],
            [ "zone" => ".info.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".info.ve", "host" => "whois.nic.ve" ],
            [ "zone" => ".ing", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".ink", "host" => "whois.centralnic.com" ],
            [ "zone" => ".ink", "host" => "whois.donuts.co" ],
            [ "zone" => ".institute", "host" => "whois.donuts.co" ],
            [ "zone" => ".insure", "host" => "whois.donuts.co" ],
            [ "zone" => ".int", "host" => "whois.iana.org" ],
            [ "zone" => ".int", "host" => "whois.isi.edu" ],
            [ "zone" => ".international", "host" => "whois.donuts.co" ],
            [ "zone" => ".investments", "host" => "whois.donuts.co" ],
            [ "zone" => ".io", "host" => "whois.nic.io" ],
            [ "zone" => ".iq", "host" => "whois.cmc.iq" ],
            [ "zone" => ".ir", "host" => "whois.nic.ir" ],
            [ "zone" => ".is", "host" => "whois.isnic.is" ],
            [ "zone" => ".it", "host" => "whois.nic.it" ],
            [ "zone" => ".iwi.nz", "host" => "whois.srs.net.nz" ],
            [ "zone" => ".java", "host" => "whois.nic.java" ],
            [ "zone" => ".jcb", "host" => "whois.nic.gmo" ],
            [ "zone" => ".je", "host" => "whois.je" ],
            [ "zone" => ".jetzt", "host" => "whois.nic.jetzt" ],
            [ "zone" => ".jl.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".jobs", "host" => "jobswhois.verisign-grs.com" ],
            [ "zone" => ".joburg", "host" => "joburg-whois.registry.net.za" ],
            [ "zone" => ".jor.br", "host" => "whois.nic.br" ],
            [ "zone" => ".jp", "host" => "whois.jprs.jp" ],
            [ "zone" => ".jpn.com", "host" => "whois.centralnic.net" ],
            [ "zone" => ".js.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".juegos", "host" => "whois.uniregistry.net" ],
            [ "zone" => ".k12.il", "host" => "whois.isoc.org.il" ],
            [ "zone" => ".k12.tr", "host" => "whois.metu.edu.tr" ],
            [ "zone" => ".kaufen", "host" => "whois.unitedtld.com" ],
            [ "zone" => ".kddi", "host" => "whois.nic.kddi" ],
            [ "zone" => ".ke", "host" => "whois.kenic.or.ke" ],
            [ "zone" => ".kg", "host" => "whois.domain.kg" ],
            [ "zone" => ".kh.ua", "host" => "whois.net.ua" ],
            [ "zone" => ".ki", "host" => "whois.nic.ki" ],
            [ "zone" => ".kiev.ua", "host" => "whois.net.ua" ],
            [ "zone" => ".kim", "host" => "whois.afilias.net" ],
            [ "zone" => ".kitchen", "host" => "whois.donuts.co" ],
            [ "zone" => ".kiwi", "host" => "whois.dot-kiwi.com" ],
            [ "zone" => ".kiwi", "host" => "whois.nic.kiwi" ],
            [ "zone" => ".kiwi.nz", "host" => "whois.srs.net.nz" ],
            [ "zone" => ".koeln", "host" => "whois-fe1.pdt.koeln.tango.knipp.de" ],
            [ "zone" => ".kr", "host" => "whois.kr", "parser_type" => "common_flat" ],
            [ "zone" => ".kr", "host" => "whois.nic.or.kr", "parser_type" => "common_flat" ],
            [ "zone" => ".kr.com", "host" => "whois.centralnic.net" ],
            [ "zone" => ".krd", "host" => "whois.aridnrs.net.au" ],
            [ "zone" => ".kred", "host" => "whois.nic.kred" ],
            [ "zone" => ".ky", "host" => "whois.kyregistry.ky" ],
            [ "zone" => ".kyoto", "host" => "whois.nic.kyoto" ],
            [ "zone" => ".kz", "host" => "whois.nic.kz" ],
            [ "zone" => ".la", "host" => "whois.nic.la" ],
            [ "zone" => ".la", "host" => "whois2.afilias-grs.net" ],
            [ "zone" => ".lacaixa", "host" => "whois.nic.lacaixa" ],
            [ "zone" => ".land", "host" => "whois.donuts.co" ],
            [ "zone" => ".lat", "host" => "whois.nic.lat" ],
            [ "zone" => ".latrobe", "host" => "whois.nic.latrobe" ],
            [ "zone" => ".lawyer", "host" => "whois.rightside.co" ],
            [ "zone" => ".lc", "host" => "whois2.afilias-grs.net" ],
            [ "zone" => ".lease", "host" => "whois.donuts.co" ],
            [ "zone" => ".leclerc", "host" => "whois-leclerc.nic.fr" ],
            [ "zone" => ".legal", "host" => "whois.donuts.co" ],
            [ "zone" => ".lel.br", "host" => "whois.nic.br" ],
            [ "zone" => ".lg.ua", "host" => "whois.net.ua" ],
            [ "zone" => ".lgbt", "host" => "whois.afilias.net" ],
            [ "zone" => ".li", "host" => "whois.nic.li" ],
            [ "zone" => ".life", "host" => "whois.donuts.co" ],
            [ "zone" => ".lighting", "host" => "whois.donuts.co" ],
            [ "zone" => ".limited", "host" => "whois.donuts.co" ],
            [ "zone" => ".limo", "host" => "whois.donuts.co" ],
            [ "zone" => ".link", "host" => "whois.uniregistry.net" ],
            [ "zone" => ".ln.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".loans", "host" => "whois.donuts.co" ],
            [ "zone" => ".london", "host" => "whois-lon.mm-registry.com" ],
            [ "zone" => ".london", "host" => "whois.nic.london" ],
            [ "zone" => ".lotte", "host" => "whois.nic.lotte" ],
            [ "zone" => ".lotto", "host" => "whois.afilias.net" ],
            [ "zone" => ".lt", "host" => "whois.domreg.lt" ],
            [ "zone" => ".ltd.uk", "host" => "whois.nic.uk" ],
            [ "zone" => ".ltda", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".lu", "host" => "whois.dns.lu" ],
            [ "zone" => ".lu", "host" => "whois.restena.lu" ],
            [ "zone" => ".luxe", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".luxury", "host" => "whois.donuts.co" ],
            [ "zone" => ".luxury", "host" => "whois.nic.luxury" ],
            [ "zone" => ".lv", "host" => "whois.nic.lv" ],
            [ "zone" => ".lviv.ua", "host" => "whois.net.ua" ],
            [ "zone" => ".ly", "host" => "whois.lydomains.com" ],
            [ "zone" => ".ly", "host" => "whois.nic.ly" ],
            [ "zone" => ".ma", "host" => "whois.iam.net.ma" ],
            [ "zone" => ".madrid", "host" => "whois.madrid.rs.corenic.net" ],
            [ "zone" => ".mail.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".maison", "host" => "whois.donuts.co" ],
            [ "zone" => ".management", "host" => "whois.donuts.co" ],
            [ "zone" => ".mango", "host" => "whois.mango.coreregistry.net" ],
            [ "zone" => ".maori.nz", "host" => "whois.srs.net.nz" ],
            [ "zone" => ".market", "host" => "whois.rightside.co" ],
            [ "zone" => ".marketing", "host" => "whois.donuts.co" ],
            [ "zone" => ".mc", "host" => "whois.ripe.net" ],
            [ "zone" => ".md", "host" => "whois.nic.md" ],
            [ "zone" => ".me", "host" => "whois.meregistry.net" ],
            [ "zone" => ".me", "host" => "whois.nic.me" ],
            [ "zone" => ".me.uk", "host" => "whois.nic.uk" ],
            [ "zone" => ".med.br", "host" => "whois.nic.br" ],
            [ "zone" => ".med.ec", "host" => "whois.lac.net" ],
            [ "zone" => ".media", "host" => "whois.donuts.co" ],
            [ "zone" => ".media.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".meet", "host" => "whois.afilias.net" ],
            [ "zone" => ".melbourne", "host" => "whois.aridnrs.net.au" ],
            [ "zone" => ".meme", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".memorial", "host" => "whois.donuts.co" ],
            [ "zone" => ".menu", "host" => "whois.nic.menu" ],
            [ "zone" => ".mg", "host" => "whois.nic.mg" ],
            [ "zone" => ".mi.th", "host" => "whois.thnic.net" ],
            [ "zone" => ".miami", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".miasta.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".mil", "host" => "whois.internic.net" ],
            [ "zone" => ".mil", "host" => "whois.nic.mil" ],
            [ "zone" => ".mil.br", "host" => "whois.nic.br" ],
            [ "zone" => ".mil.ec", "host" => "whois.lac.net" ],
            [ "zone" => ".mil.id", "host" => "whois.idnic.net.id" ],
            [ "zone" => ".mil.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".mil.tr", "host" => "whois.metu.edu.tr" ],
            [ "zone" => ".mil.za", "host" => "whois.co.za" ],
            [ "zone" => ".mini", "host" => "whois.ksregistry.net" ],
            [ "zone" => ".mk", "host" => "whois.marnet.mk" ],
            [ "zone" => ".mk", "host" => "whois.ripe.net" ],
            [ "zone" => ".ml", "host" => "whois.dot.ml" ],
            [ "zone" => ".mn", "host" => "whois.afilias-grs.info" ],
            [ "zone" => ".mn", "host" => "whois.nic.mn" ],
            [ "zone" => ".mo", "host" => "whois.monic.mo" ],
            [ "zone" => ".mo.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".mobi", "host" => "whois.dotmobiregistry.net" ],
            [ "zone" => ".moda", "host" => "whois.unitedtld.com" ],
            [ "zone" => ".monash", "host" => "whois.nic.monash" ],
            [ "zone" => ".money", "host" => "whois.donuts.co" ],
            [ "zone" => ".mortgage", "host" => "whois.rightside.co" ],
            [ "zone" => ".moscow", "host" => "whois.nic.moscow" ],
            [ "zone" => ".motorcycles", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".mov", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".mp", "host" => "whois.nic.mp" ],
            [ "zone" => ".ms", "host" => "whois.nic.ms" ],
            [ "zone" => ".msk.ru", "host" => "whois.nic.ru" ],
            [ "zone" => ".mtpc", "host" => "whois.nic.gmo" ],
            [ "zone" => ".mu", "host" => "whois.nic.mu", "parser_type" => "common_flat" ],
            [ "zone" => ".muni.il", "host" => "whois.isoc.org.il" ],
            [ "zone" => ".museum", "host" => "whois.museum" ],
            [ "zone" => ".mx", "host" => "whois.mx" ],
            [ "zone" => ".mx", "host" => "whois.nic.mx" ],
            [ "zone" => ".my", "host" => "whois.mynic.my" ],
            [ "zone" => ".my", "host" => "whois.mynic.net.my" ],
            [ "zone" => ".mz", "host" => "whois.nic.mz" ],
            [ "zone" => ".na", "host" => "whois.na-nic.com.na" ],
            [ "zone" => ".nagoya", "host" => "whois.gmoregistry.net" ],
            [ "zone" => ".name", "host" => "whois.nic.name" ],
            [ "zone" => ".navy", "host" => "whois.rightside.co" ],
            [ "zone" => ".nc", "host" => "whois.nc" ],
            [ "zone" => ".ne.jp", "host" => "whois.nic.ad.jp" ],
            [ "zone" => ".net", "host" => "whois.crsnic.net" ],
            [ "zone" => ".net", "host" => "whois.verisign-grs.com" ],
            [ "zone" => ".net.br", "host" => "whois.nic.br" ],
            [ "zone" => ".net.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".net.co", "host" => "whois.nic.co" ],
            [ "zone" => ".net.ec", "host" => "whois.lac.net" ],
            [ "zone" => ".net.hk", "host" => "whois.hkdnr.net.hk" ],
            [ "zone" => ".net.il", "host" => "whois.isoc.org.il" ],
            [ "zone" => ".net.in", "host" => "whois.inregistry.in" ],
            [ "zone" => ".net.mm", "host" => "whois.nic.mm" ],
            [ "zone" => ".net.mx", "host" => "whois.nic.mx" ],
            [ "zone" => ".net.my", "host" => "whois.mynic.net.my" ],
            [ "zone" => ".net.nz", "host" => "whois.srs.net.nz" ],
            [ "zone" => ".net.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".net.ru", "host" => "whois.nic.ru" ],
            [ "zone" => ".net.ru", "host" => "whois.ripn.net" ],
            [ "zone" => ".net.sg", "host" => "whois.nic.net.sg" ],
            [ "zone" => ".net.th", "host" => "whois.thnic.net" ],
            [ "zone" => ".net.tr", "host" => "whois.metu.edu.tr" ],
            [ "zone" => ".net.tw", "host" => "whois.twnic.net" ],
            [ "zone" => ".net.ua", "host" => "whois.net.ua" ],
            [ "zone" => ".net.uk", "host" => "whois.nic.uk" ],
            [ "zone" => ".net.ve", "host" => "whois.nic.ve" ],
            [ "zone" => ".net.za", "host" => "whois.co.za" ],
            [ "zone" => ".network", "host" => "whois.donuts.co" ],
            [ "zone" => ".new", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".nexus", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".nf", "host" => "whois.nic.cx" ],
            [ "zone" => ".nf", "host" => "whois.nic.nf" ],
            [ "zone" => ".ng", "host" => "whois.nic.net.ng" ],
            [ "zone" => ".ngo", "host" => "whois.publicinterestregistry.net" ],
            [ "zone" => ".ngo.za", "host" => "whois.co.za" ],
            [ "zone" => ".nico", "host" => "whois.nic.nico" ],
            [ "zone" => ".ninja", "host" => "whois.donuts.co" ],
            [ "zone" => ".ninja", "host" => "whois.unitedtld.com" ],
            [ "zone" => ".nissan", "host" => "whois.nic.gmo" ],
            [ "zone" => ".nl", "host" => "whois.domain-registry.nl" ],
            [ "zone" => ".nm.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".no", "host" => "whois.norid.no" ],
            [ "zone" => ".no.com", "host" => "whois.centralnic.com" ],
            [ "zone" => ".no.com", "host" => "whois.centralnic.net" ],
            [ "zone" => ".nom.br", "host" => "whois.nic.br" ],
            [ "zone" => ".nom.co", "host" => "whois.nic.co" ],
            [ "zone" => ".nom.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".nom.za", "host" => "whois.co.za" ],
            [ "zone" => ".nra", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".nrw", "host" => "whois-fe1.pdt.nrw.tango.knipp.de" ],
            [ "zone" => ".nrw", "host" => "whois.nic.nrw" ],
            [ "zone" => ".ntr.br", "host" => "whois.nic.br" ],
            [ "zone" => ".nu", "host" => "whois.iis.nu" ],
            [ "zone" => ".nu", "host" => "whois.nic.nu" ],
            [ "zone" => ".nx.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".nyc", "host" => "whois.nic.nyc" ],
            [ "zone" => ".nz", "host" => "whois.srs.net.nz" ],
            [ "zone" => ".odo.br", "host" => "whois.nic.br" ],
            [ "zone" => ".okinawa", "host" => "whois.gmoregistry.ne" ],
            [ "zone" => ".om", "host" => "whois.registry.om", "parser_type" => "common_flat" ],
            [ "zone" => ".one", "host" => "whois.nic.one" ],
            [ "zone" => ".ong", "host" => "whois.publicinterestregistry.net" ],
            [ "zone" => ".onl", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".ooo", "host" => "whois.nic.ooo" ],
            [ "zone" => ".or.ac", "host" => "whois.nic.ac" ],
            [ "zone" => ".or.at", "host" => "whois.nic.at" ],
            [ "zone" => ".or.jp", "host" => "whois.nic.ad.jp" ],
            [ "zone" => ".or.th", "host" => "whois.thnic.net" ],
            [ "zone" => ".org", "host" => "whois.pir.org" ],
            [ "zone" => ".org", "host" => "whois.publicinterestregistry.net" ],
            [ "zone" => ".org.br", "host" => "whois.nic.br" ],
            [ "zone" => ".org.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".org.ec", "host" => "whois.lac.net" ],
            [ "zone" => ".org.hk", "host" => "whois.hkdnr.net.hk" ],
            [ "zone" => ".org.il", "host" => "whois.isoc.org.il" ],
            [ "zone" => ".org.in", "host" => "whois.inregistry.in" ],
            [ "zone" => ".org.mm", "host" => "whois.nic.mm" ],
            [ "zone" => ".org.mx", "host" => "whois.nic.mx" ],
            [ "zone" => ".org.my", "host" => "whois.mynic.net.my" ],
            [ "zone" => ".org.nz", "host" => "whois.srs.net.nz" ],
            [ "zone" => ".org.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".org.ru", "host" => "whois.nic.ru" ],
            [ "zone" => ".org.sg", "host" => "whois.nic.net.sg" ],
            [ "zone" => ".org.tr", "host" => "whois.metu.edu.tr" ],
            [ "zone" => ".org.tw", "host" => "whois.twnic.net" ],
            [ "zone" => ".org.ua", "host" => "whois.net.ua" ],
            [ "zone" => ".org.uk", "host" => "whois.nic.uk" ],
            [ "zone" => ".org.ve", "host" => "whois.nic.ve" ],
            [ "zone" => ".organic", "host" => "whois.afilias.net" ],
            [ "zone" => ".ovh", "host" => "whois-ovh.nic.fr" ],
            [ "zone" => ".paris", "host" => "whois-paris.nic.fr" ],
            [ "zone" => ".partners", "host" => "whois.donuts.co" ],
            [ "zone" => ".parts", "host" => "whois.donuts.co" ],
            [ "zone" => ".pc.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".pe", "host" => "kero.yachay.pe" ],
            [ "zone" => ".pf", "host" => "whois.registry.pf" ],
            [ "zone" => ".photo", "host" => "whois.uniregistry.net" ],
            [ "zone" => ".photography", "host" => "whois.donuts.co" ],
            [ "zone" => ".photos", "host" => "whois.donuts.co" ],
            [ "zone" => ".physio", "host" => "whois.nic.physio" ],
            [ "zone" => ".pics", "host" => "whois.uniregistry.net" ],
            [ "zone" => ".pictures", "host" => "whois.donuts.co" ],
            [ "zone" => ".pink", "host" => "whois.afilias.net" ],
            [ "zone" => ".pink", "host" => "whois.donuts.co" ],
            [ "zone" => ".pizza", "host" => "whois.donuts.co" ],
            [ "zone" => ".pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".place", "host" => "whois.donuts.co" ],
            [ "zone" => ".plc.uk", "host" => "whois.nic.uk" ],
            [ "zone" => ".plumbing", "host" => "whois.donuts.co" ],
            [ "zone" => ".pm", "host" => "whois.nic.pm" ],
            [ "zone" => ".pohl", "host" => "whois.ksregistry.net" ],
            [ "zone" => ".poker", "host" => "whois.afilias.net" ],
            [ "zone" => ".porn", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".post", "host" => "whois.dotpostregistry.net" ],
            [ "zone" => ".pp.ru", "host" => "whois.nic.ru" ],
            [ "zone" => ".ppg.br", "host" => "whois.nic.br" ],
            [ "zone" => ".pr", "host" => "whois.nic.pr" ],
            [ "zone" => ".press", "host" => "whois.centralnic.com" ],
            [ "zone" => ".press", "host" => "whois.nic.press" ],
            [ "zone" => ".presse.fr", "host" => "whois.nic.fr" ],
            [ "zone" => ".priv.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".pro", "host" => "whois.dotproregistry.net" ],
            [ "zone" => ".pro", "host" => "whois.registrypro.pro" ],
            [ "zone" => ".pro.br", "host" => "whois.nic.br" ],
            [ "zone" => ".prod", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".productions", "host" => "whois.donuts.co" ],
            [ "zone" => ".prof", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".properties", "host" => "whois.donuts.co" ],
            [ "zone" => ".property", "host" => "whois.uniregistry.net" ],
            [ "zone" => ".ps", "host" => "whois.pnina.ps" ],
            [ "zone" => ".psc.br", "host" => "whois.nic.br" ],
            [ "zone" => ".psi.br", "host" => "whois.nic.br" ],
            [ "zone" => ".pt", "host" => "whois.dns.pt" ],
            [ "zone" => ".pub", "host" => "whois.unitedtld.com" ],
            [ "zone" => ".pw", "host" => "whois.nic.pw" ],
            [ "zone" => ".qa", "host" => "whois.registry.qa", "parser_type" => "common_flat" ],
            [ "zone" => ".qc.com", "host" => "whois.centralnic.com" ],
            [ "zone" => ".qc.com", "host" => "whois.centralnic.net" ],
            [ "zone" => ".qh.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".qpon", "host" => "whois.nic.qpon" ],
            [ "zone" => ".quebec", "host" => "whois.nic.quebec" ],
            [ "zone" => ".quebec", "host" => "whois.quebec.rs.corenic.net" ],
            [ "zone" => ".re", "host" => "whois.nic.re" ],
            [ "zone" => ".realestate.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".rec.br", "host" => "whois.nic.br" ],
            [ "zone" => ".recipes", "host" => "whois.donuts.co" ],
            [ "zone" => ".red", "host" => "whois.afilias.net" ],
            [ "zone" => ".rehab", "host" => "whois.rightside.co" ],
            [ "zone" => ".reise", "host" => "whois.nic.reise" ],
            [ "zone" => ".reisen", "host" => "whois.donuts.co" ],
            [ "zone" => ".reit", "host" => "whois.nic.reit" ],
            [ "zone" => ".rel.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".rentals", "host" => "whois.donuts.co" ],
            [ "zone" => ".repair", "host" => "whois.donuts.co" ],
            [ "zone" => ".report", "host" => "whois.donuts.co" ],
            [ "zone" => ".republican", "host" => "whois.rightside.co" ],
            [ "zone" => ".res.in", "host" => "whois.inregistry.in" ],
            [ "zone" => ".rest", "host" => "whois.centralnic.com" ],
            [ "zone" => ".restaurant", "host" => "whois.donuts.co" ],
            [ "zone" => ".reviews", "host" => "whois.unitedtld.com" ],
            [ "zone" => ".rich", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".rio", "host" => "whois.gtlds.nic.br" ],
            [ "zone" => ".rip", "host" => "whois.rightside.co" ],
            [ "zone" => ".ro", "host" => "whois.rotld.ro", "parser_type" => "common_flat" ],
            [ "zone" => ".rocks", "host" => "whois.unitedtld.com" ],
            [ "zone" => ".rodeo", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".rs", "host" => "whois.rnids.rs", "parser_type" => "common_flat" ],
            [ "zone" => ".rsvp", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".ru", "host" => "whois.ripn.net" ],
            [ "zone" => ".ru", "host" => "whois.tcinet.ru" ],
            [ "zone" => ".ru.com", "host" => "whois.centralnic.net" ],
            [ "zone" => ".ruhr", "host" => "whois.nic.ruhr" ],
            [ "zone" => ".rw", "host" => "whois.ricta.org.rw" ],
            [ "zone" => ".sa", "host" => "saudinic.net.sa" ],
            [ "zone" => ".sa", "host" => "whois.nic.net.sa" ],
            [ "zone" => ".sa.com", "host" => "whois.centralnic.com" ],
            [ "zone" => ".sa.com", "host" => "whois.centralnic.net" ],
            [ "zone" => ".saarland", "host" => "whois.ksregistry.net" ],
            [ "zone" => ".sale", "host" => "whois.rightside.co" ],
            [ "zone" => ".samsung", "host" => "whois.nic.samsung" ],
            [ "zone" => ".sarl", "host" => "whois.donuts.co" ],
            [ "zone" => ".saxo", "host" => "whois.aridnrs.net.au" ],
            [ "zone" => ".sb", "host" => "whois.nic.net.sb" ],
            [ "zone" => ".sc", "host" => "wawa.eahd.or.ug" ],
            [ "zone" => ".sc", "host" => "whois2.afilias-grs.net" ],
            [ "zone" => ".sc.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".sca", "host" => "whois.nic.sca" ],
            [ "zone" => ".scb", "host" => "whois.nic.scb" ],
            [ "zone" => ".schmidt", "host" => "whois.nic.schmidt" ],
            [ "zone" => ".school", "host" => "whois.donuts.co" ],
            [ "zone" => ".school.nz", "host" => "whois.srs.net.nz" ],
            [ "zone" => ".school.za", "host" => "whois.co.za" ],
            [ "zone" => ".schule", "host" => "whois.donuts.co" ],
            [ "zone" => ".scot", "host" => "whois.scot.coreregistry.net" ],
            [ "zone" => ".se", "host" => "whois.iis.se" ],
            [ "zone" => ".se", "host" => "whois.nic-se.se" ],
            [ "zone" => ".se.com", "host" => "whois.centralnic.com" ],
            [ "zone" => ".se.com", "host" => "whois.centralnic.net" ],
            [ "zone" => ".se.net", "host" => "whois.centralnic.com" ],
            [ "zone" => ".se.net", "host" => "whois.centralnic.net" ],
            [ "zone" => ".services", "host" => "whois.donuts.co" ],
            [ "zone" => ".sexy", "host" => "whois.uniregistry.net" ],
            [ "zone" => ".sg", "host" => "whois.nic.net.sg" ],
            [ "zone" => ".sg", "host" => "whois.sgnic.sg" ],
            [ "zone" => ".sh", "host" => "whois.nic.sh" ],
            [ "zone" => ".sh.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".shiksha", "host" => "whois.afilias.net" ],
            [ "zone" => ".shoes", "host" => "whois.donuts.co" ],
            [ "zone" => ".shop.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".si", "host" => "whois.arnes.si" ],
            [ "zone" => ".singles", "host" => "whois.donuts.co" ],
            [ "zone" => ".sk", "host" => "whois.sk-nic.sk" ],
            [ "zone" => ".sklep.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".sky", "host" => "whois.nic.sky" ],
            [ "zone" => ".sl", "host" => "whois.nic.sl" ],
            [ "zone" => ".slg.br", "host" => "whois.nic.br" ],
            [ "zone" => ".sm", "host" => "whois.nic.sm" ],
            [ "zone" => ".sn", "host" => "whois.nic.sn" ],
            [ "zone" => ".sn.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".so", "host" => "whois.nic.so" ],
            [ "zone" => ".sochi.su", "host" => "whois.nic.ru" ],
            [ "zone" => ".social", "host" => "whois.unitedtld.com" ],
            [ "zone" => ".software", "host" => "whois.rightside.co" ],
            [ "zone" => ".sohu", "host" => "whois.gtld.knet.cn" ],
            [ "zone" => ".solar", "host" => "whois.donuts.co" ],
            [ "zone" => ".solutions", "host" => "whois.donuts.co" ],
            [ "zone" => ".sos.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".soy", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".space", "host" => "whois.nic.space" ],
            [ "zone" => ".spb.ru", "host" => "whois.nic.ru" ],
            [ "zone" => ".spiegel", "host" => "whois.ksregistry.net" ],
            [ "zone" => ".st", "host" => "whois.nic.st" ],
            [ "zone" => ".study", "host" => "whois.nic.study" ],
            [ "zone" => ".style", "host" => "whois.donuts.co" ],
            [ "zone" => ".su", "host" => "whois.ripn.net" ],
            [ "zone" => ".su", "host" => "whois.tcinet.ru" ],
            [ "zone" => ".sucks", "host" => "whois.nic.sucks" ],
            [ "zone" => ".supplies", "host" => "whois.donuts.co" ],
            [ "zone" => ".supply", "host" => "whois.donuts.co" ],
            [ "zone" => ".support", "host" => "whois.donuts.co" ],
            [ "zone" => ".surf", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".surgery", "host" => "whois.donuts.co" ],
            [ "zone" => ".sx", "host" => "whois.sx" ],
            [ "zone" => ".sy", "host" => "whois.tld.sy" ],
            [ "zone" => ".sydney", "host" => "whois.nic.sydney" ],
            [ "zone" => ".systems", "host" => "whois.donuts.co" ],
            [ "zone" => ".taipei", "host" => "whois.nic.taipei" ],
            [ "zone" => ".targi.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".tatar", "host" => "whois.nic.tatar" ],
            [ "zone" => ".tattoo", "host" => "whois.uniregistry.net" ],
            [ "zone" => ".tax", "host" => "whois.donuts.co" ],
            [ "zone" => ".tc", "host" => "whois.adamsnames.tc" ],
            [ "zone" => ".tc", "host" => "whois.meridiantld.net" ],
            [ "zone" => ".technology", "host" => "whois.donuts.co" ],
            [ "zone" => ".tel", "host" => "whois.nic.tel" ],
            [ "zone" => ".tennis", "host" => "whois.donuts.co" ],
            [ "zone" => ".tf", "host" => "whois.nic.tf" ],
            [ "zone" => ".tg", "host" => "whois.nic.tg" ],
            [ "zone" => ".th", "host" => "whois.thnic.co.th" ],
            [ "zone" => ".th", "host" => "whois.thnic.net" ],
            [ "zone" => ".tienda", "host" => "whois.donuts.co" ],
            [ "zone" => ".tips", "host" => "whois.donuts.co" ],
            [ "zone" => ".tires", "host" => "whois.donuts.co" ],
            [ "zone" => ".tirol", "host" => "whois.nic.tirol" ],
            [ "zone" => ".tj.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".tk", "host" => "whois.dot.tk" ],
            [ "zone" => ".tl", "host" => "whois.domains.tl" ],
            [ "zone" => ".tl", "host" => "whois.nic.tl" ],
            [ "zone" => ".tm", "host" => "whois.nic.tm", "parser_type" => "common_flat" ],
            [ "zone" => ".tm.fr", "host" => "whois.nic.fr" ],
            [ "zone" => ".tm.mc", "host" => "whois.ripe.net" ],
            [ "zone" => ".tm.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".tm.za", "host" => "whois.co.za" ],
            [ "zone" => ".tmp.br", "host" => "whois.nic.br" ],
            [ "zone" => ".tn", "host" => "whois.ati.tn" ],
            [ "zone" => ".to", "host" => "monarch.tonic.to" ],
            [ "zone" => ".to", "host" => "whois.tonic.to" ],
            [ "zone" => ".today", "host" => "whois.donuts.co" ],
            [ "zone" => ".tokyo", "host" => "whois.nic.tokyo" ],
            [ "zone" => ".tools", "host" => "whois.donuts.co" ],
            [ "zone" => ".top", "host" => "whois.nic.top" ],
            [ "zone" => ".toshiba", "host" => "whois.nic.toshiba" ],
            [ "zone" => ".tourism.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".town", "host" => "whois.donuts.co" ],
            [ "zone" => ".toys", "host" => "whois.donuts.co" ],
            [ "zone" => ".tp", "host" => "whois.domains.tl" ],
            [ "zone" => ".tr", "host" => "whois.nic.tr" ],
            [ "zone" => ".trade", "host" => "whois.nic.trade" ],
            [ "zone" => ".training", "host" => "whois.donuts.co" ],
            [ "zone" => ".travel", "host" => "whois.nic.travel" ],
            [ "zone" => ".travel.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".trust", "host" => "whois.nic.trust" ],
            [ "zone" => ".tui", "host" => "whois.ksregistry.net" ],
            [ "zone" => ".tur.br", "host" => "whois.nic.br" ],
            [ "zone" => ".turystyka.pl", "host" => "whois.dns.pl" ],
            [ "zone" => ".tv", "host" => "whois.nic.tv" ],
            [ "zone" => ".tv.br", "host" => "whois.nic.br" ],
            [ "zone" => ".tw", "host" => "whois.twnic.net.tw" ],
            [ "zone" => ".tw.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".tz", "host" => "whois.tznic.or.tz" ],
            [ "zone" => ".ua", "host" => "whois.net.ua" ],
            [ "zone" => ".ua", "host" => "whois.ua" ],
            [ "zone" => ".ug", "host" => "whois.co.ug" ],
            [ "zone" => ".uk", "host" => "whois.nic.uk" ],
            [ "zone" => ".uk.co", "host" => "whois.uk.co" ],
            [ "zone" => ".uk.com", "host" => "whois.centralnic.com" ],
            [ "zone" => ".uk.com", "host" => "whois.centralnic.net" ],
            [ "zone" => ".uk.net", "host" => "whois.centralnic.com" ],
            [ "zone" => ".uk.net", "host" => "whois.centralnic.net" ],
            [ "zone" => ".university", "host" => "whois.donuts.co" ],
            [ "zone" => ".uno", "host" => "whois.nic.uno" ],
            [ "zone" => ".uno", "host" => "whois.uno.nic" ],
            [ "zone" => ".uol", "host" => "whois.gtlds.nic.br" ],
            [ "zone" => ".us", "host" => "whois.nic.us" ],
            [ "zone" => ".us.com", "host" => "whois.centralnic.com" ],
            [ "zone" => ".us.com", "host" => "whois.centralnic.net" ],
            [ "zone" => ".uy", "host" => "whois.nic.org.uy" ],
            [ "zone" => ".uy.com", "host" => "whois.centralnic.com" ],
            [ "zone" => ".uy.com", "host" => "whois.centralnic.net" ],
            [ "zone" => ".uz", "host" => "whois.cctld.uz" ],
            [ "zone" => ".va", "host" => "whois.ripe.net" ],
            [ "zone" => ".vacations", "host" => "whois.donuts.co" ],
            [ "zone" => ".vc", "host" => "whois.adamsnames.tc" ],
            [ "zone" => ".vc", "host" => "whois2.afilias-grs.net" ],
            [ "zone" => ".ve", "host" => "whois.nic.ve" ],
            [ "zone" => ".vegas", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".ventures", "host" => "whois.donuts.co" ],
            [ "zone" => ".versicherung", "host" => "whois.nic.versicherung" ],
            [ "zone" => ".vet", "host" => "whois.rightside.co" ],
            [ "zone" => ".vet.br", "host" => "whois.nic.br" ],
            [ "zone" => ".vg", "host" => "ccwhois.ksregistry.net", "parser_type" => "common_flat" ],
            [ "zone" => ".vg", "host" => "whois.adamsnames.tc", "parser_type" => "common_flat" ],
            [ "zone" => ".viajes", "host" => "whois.donuts.co" ],
            [ "zone" => ".video", "host" => "whois.rightside.co" ],
            [ "zone" => ".villas", "host" => "whois.donuts.co" ],
            [ "zone" => ".vision", "host" => "whois.donuts.co" ],
            [ "zone" => ".vlaanderen", "host" => "whois.nic.vlaanderen" ],
            [ "zone" => ".vodka", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".vote", "host" => "whois.afilias.net" ],
            [ "zone" => ".voting", "host" => "whois.voting.tld-box.at" ],
            [ "zone" => ".voto", "host" => "whois.afilias.net" ],
            [ "zone" => ".voyage", "host" => "whois.donuts.co" ],
            [ "zone" => ".vu", "host" => "vunic.vu" ],
            [ "zone" => ".wales", "host" => "whois.nic.wales" ],
            [ "zone" => ".wang", "host" => "whois.gtld.knet.cn" ],
            [ "zone" => ".wang", "host" => "whois.nic.wang" ],
            [ "zone" => ".watch", "host" => "whois.donuts.co" ],
            [ "zone" => ".web.ve", "host" => "whois.nic.ve" ],
            [ "zone" => ".web.za", "host" => "whois.co.za" ],
            [ "zone" => ".webcam", "host" => "whois.nic.webcam" ],
            [ "zone" => ".website", "host" => "whois.nic.website" ],
            [ "zone" => ".wed", "host" => "whois.nic.wed" ],
            [ "zone" => ".wedding", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".wf", "host" => "whois.nic.wf" ],
            [ "zone" => ".whoswho", "host" => "whois.nic.whoswho" ],
            [ "zone" => ".wien", "host" => "whois.nic.wien" ],
            [ "zone" => ".wiki", "host" => "whois.nic.wiki" ],
            [ "zone" => ".wme", "host" => "whois.nic.wme" ],
            [ "zone" => ".work", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".works", "host" => "whois.donuts.co" ],
            [ "zone" => ".world", "host" => "whois.donuts.co" ],
            [ "zone" => ".ws", "host" => "whois.website.ws" ],
            [ "zone" => ".wtc", "host" => "whois.nic.wtc" ],
            [ "zone" => ".wtf", "host" => "whois.donuts.co" ],
            [ "zone" => ".xj.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".xn--1qqw23a", "host" => "whois.ngtld.cn" ],
            [ "zone" => ".xn--3bst00m", "host" => "whois.gtld.knet.cn" ],
            [ "zone" => ".xn--3ds443g", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".xn--3e0b707e", "host" => "whois.kr", "parser_type" => "common_flat" ],
            [ "zone" => ".xn--45q11c", "host" => "whois.nic.xn--45q11c" ],
            [ "zone" => ".xn--4gbrim", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".xn--55qw42g", "host" => "whois.conac.cn" ],
            [ "zone" => ".xn--55qx5d", "host" => "whois.ngtld.cn" ],
            [ "zone" => ".xn--6frz82g", "host" => "whois.afilias.net" ],
            [ "zone" => ".xn--6qq986b3xl", "host" => "whois.gtld.knet.cn" ],
            [ "zone" => ".xn--80adxhks", "host" => "whois.nic.xn--80adxhks" ],
            [ "zone" => ".xn--80ao21a", "host" => "whois.nic.kz" ],
            [ "zone" => ".xn--80asehdb", "host" => "whois.online.rs.corenic.net" ],
            [ "zone" => ".xn--80aswg", "host" => "whois.site.rs.corenic.net" ],
            [ "zone" => ".xn--c1avg", "host" => "whois.publicinterestregistry.net" ],
            [ "zone" => ".xn--cg4bki", "host" => "whois.kr", "parser_type" => "common_flat" ],
            [ "zone" => ".xn--clchc0ea0b2g2a9gcd", "host" => "whois.sgnic.sg" ],
            [ "zone" => ".xn--czrs0t", "host" => "whois.donuts.co" ],
            [ "zone" => ".xn--czru2d", "host" => "whois.gtld.knet.cn" ],
            [ "zone" => ".xn--d1acj3b", "host" => "whois.nic.xn--d1acj3b" ],
            [ "zone" => ".xn--d1alf", "host" => "whois.marnet.mk" ],
            [ "zone" => ".xn--fiq228c5hs", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".xn--fiq64b", "host" => "whois.gtld.knet.cn" ],
            [ "zone" => ".xn--fiqs8s", "host" => "cwhois.cnnic.cn" ],
            [ "zone" => ".xn--fiqz9s", "host" => "cwhois.cnnic.cn" ],
            [ "zone" => ".xn--flw351e", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".xn--hxt814e", "host" => "whois.nic.xn--hxt814e" ],
            [ "zone" => ".xn--i1b6b1a6a2e", "host" => "whois.publicinterestregistry.net" ],
            [ "zone" => ".xn--io0a7i", "host" => "whois.ngtld.cn" ],
            [ "zone" => ".xn--j1amh", "host" => "whois.dotukr.com" ],
            [ "zone" => ".xn--j6w193g", "host" => "whois.hkirc.hk" ],
            [ "zone" => ".xn--kprw13d", "host" => "whois.twnic.net.tw" ],
            [ "zone" => ".xn--kpry57d", "host" => "whois.twnic.net.tw" ],
            [ "zone" => ".xn--kput3i", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".xn--lgbbat1ad8j", "host" => "whois.nic.dz" ],
            [ "zone" => ".xn--mgb9awbf", "host" => "whois.registry.om", "parser_type" => "common_flat" ],
            [ "zone" => ".xn--mgba3a4f16a", "host" => "whois.nic.ir" ],
            [ "zone" => ".xn--mgbaam7a8h", "host" => "whois.aeda.net.ae", "parser_type" => "common_flat" ],
            [ "zone" => ".xn--mgbab2bd", "host" => "whois.bazaar.coreregistry.net" ],
            [ "zone" => ".xn--mgberp4a5d4ar", "host" => "whois.nic.net.sa" ],
            [ "zone" => ".xn--mgbx4cd0ab", "host" => "whois.mynic.my" ],
            [ "zone" => ".xn--mxtq1m", "host" => "whois.nic.xn--mxtq1m" ],
            [ "zone" => ".xn--ngbc5azd", "host" => "whois.nic.xn--ngbc5azd" ],
            [ "zone" => ".xn--nqv7f", "host" => "whois.publicinterestregistry.net" ],
            [ "zone" => ".xn--nqv7fs00ema", "host" => "whois.publicinterestregistry.net" ],
            [ "zone" => ".xn--o3cw4h", "host" => "whois.thnic.co.th" ],
            [ "zone" => ".xn--ogbpf8fl", "host" => "whois.tld.sy" ],
            [ "zone" => ".xn--p1acf", "host" => "whois.nic.xn--p1acf" ],
            [ "zone" => ".xn--p1ai", "host" => "whois.ripn.net" ],
            [ "zone" => ".xn--p1ai", "host" => "whois.tcinet.ru" ],
            [ "zone" => ".xn--q9jyb4c", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".xn--qcka1pmc", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".xn--rhqv96g", "host" => "whois.nic.xn--rhqv96g" ],
            [ "zone" => ".xn--unup4y", "host" => "whois.donuts.co" ],
            [ "zone" => ".xn--vermgensberater-ctb", "host" => "whois.ksregistry.net" ],
            [ "zone" => ".xn--vermgensberatung-pwb", "host" => "whois.ksregistry.net" ],
            [ "zone" => ".xn--vhquv", "host" => "whois.donuts.co" ],
            [ "zone" => ".xn--wgbl6a", "host" => "whois.registry.qa", "parser_type" => "common_flat" ],
            [ "zone" => ".xn--xhq521b", "host" => "whois.ngtld.cn" ],
            [ "zone" => ".xn--yfro4i67o", "host" => "whois.sgnic.sg" ],
            [ "zone" => ".xn--ygbi2ammx", "host" => "whois.pnina.ps" ],
            [ "zone" => ".xn--zfr164b", "host" => "whois.conac.cn" ],
            [ "zone" => ".xxx", "host" => "whois.nic.xxx" ],
            [ "zone" => ".xyz", "host" => "whois.nic.xyz" ],
            [ "zone" => ".xz.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".yachts", "host" => "whois.afilias-srs.net" ],
            [ "zone" => ".yn.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".yodobashi", "host" => "whois.nic.gmo" ],
            [ "zone" => ".yoga", "host" => "whois-dub.mm-registry.com" ],
            [ "zone" => ".youtube", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".yt", "host" => "whois.nic.yt" ],
            [ "zone" => ".za.com", "host" => "whois.centralnic.com" ],
            [ "zone" => ".za.com", "host" => "whois.centralnic.net" ],
            [ "zone" => ".zip", "host" => "domain-registry-whois.l.google.com" ],
            [ "zone" => ".zj.cn", "host" => "whois.cnnic.net.cn" ],
            [ "zone" => ".zlg.br", "host" => "whois.nic.br" ],
            [ "zone" => ".zm", "host" => "whois.nic.zm" ],
            [ "zone" => ".zone", "host" => "whois.donuts.co" ],
            [ "zone" => ".zuerich", "host" => "whois.ksregistry.net" ],
        ];
    }

    /**
     * @param string $type
     * @return array
     */
    public static function getParserConfig($type = null)
    {
        if ($type == 'block') {
            return self::getBlockParserConfig();
        }
        if ($type == 'common_flat') {
            return self::getCommonFlatParserConfig();
        }
        return self::getCommonParserConfig();
    }

    /**
     * @return array
     */
    private static function getBlockParserConfig()
    {
        $cfg = self::getCommonParserConfig();
        return $cfg;
    }
    /**
     * @return array
     */
    private static function getCommonFlatParserConfig()
    {
        $cfg = self::getCommonParserConfig();
        $cfg['isFlat'] = true;
        return $cfg;
    }

    /**
     * @return array
     */
    private static function getCommonParserConfig()
    {
        return [
            'domainKeys' => [
                "domain",
                "domainname",
                "domain name",
                "query",
            ],
            'whoisServerKeys' => [
                "whois",
                "whoisserver",
                "whois server",
                "registrar whois server",
            ],
            'nameServersKeys' => [
                "nameserver",
                "name server",
                "nserver",
                "host name",
                "dns",
            ],
            'nameServersKeysGroups' => [
                [ "ns 1", "ns 2", "ns 3", "ns 4" ],
            ],
            'creationDateKeys' => [
                "creationdate",
                "creation date",
                "registration date",
                "domain registration date",
                "registration time",
                "created",
                "created on",
                "created date",
                "registered",
                "registered on",
                "registered date",
                "record created",
            ],
            'expirationDateKeys' => [
                "expirationdate",
                "expiration date",
                "expiration time",
                "exp date",
                "domain expiration date",
                "registry expiry date",
                "registrar registration expiration date",
                "expiry",
                "paid-till",
            ],
            'ownerKeys' => [
                "owner orgname",
                "owner-organization",
                "owner name",
                "owner-name",
                "organization",
                "registrant organization",
                "registrant-organization",
                "registrant internationalized organization",
                "registrant contact organisation",
                "registrant",
                "registrant name",
                "domain holder",
                "holder",
                "org",
                "tech organization",
                "admin organization",
            ],
            'registrarKeys' => [
                "registrar",
                "registrar name",
                "sponsoring registrar",
                "sponsoring registrar organization",
            ],
            'statesKeys' => [
                "domain status",
                "domainstatus",
                "status",
                "state",
            ],
            'notRegisteredStatesDict' => [
                "not registered" => 1,
                "no object found" => 1,
                "available" => 1,
                "free" => 1,
            ],
        ];
    }
}